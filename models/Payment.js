...
class Payment {
    static async create(paymentData) {
        try {
            const { user_id, course_id, amount, method, screenshot_path } = paymentData;
            ...
            return new Promise((resolve, reject) => {
                db.run(sql, [user_id, course_id, amount, method, screenshot_path], function(err) {
                    if (err) {
                        reject(err);
                    } else {
                        Payment.findById(this.lastID)
                            .then(resolve)
                            .catch(reject);
                    }
                });
